...
from utils import *
from loss import OriTripletLoss, IPDM,DCL,GE_CGA
...
parser.add_argument('--resume', '-r', default='sysu_GLCN_p4_n6_lr_0.0003_seed_0_ADMW_best', type=str, help='resume from checkpoint')
parser.add_argument('--test-only', action='store_true', help='test only')
parser.add_argument('--save_epoch', default=20, type=int,metavar='s', help='save model every 10 epochs')
parser.add_argument('--optim', default='ADMW', type=str, help='SGD,ADMW')
parser.add_argument('--model_path', default='result/GLCN/save_model/', type=str, help='model save path')
parser.add_argument('--log_path', default='result/GLCN/log/', type=str, help='log save path')
parser.add_argument('--vis_log_path', default='result/GLCN/log/vis_log/', type=str, help='log save path')
...
parser.add_argument('--method', default='GLCN', type=str,
                    metavar='m', help='method type: base or LAReViT')
parser.add_argument('--config_file', default='/root/autodl-tmp/GLCN-master/config/SYSU.yml',
...
if dataset == 'sysu':
    data_path='/root/autodl-tmp/data/SYSU-MM01/'
...
suffix = dataset
suffix = suffix + '_GLCN_p{}_n{}_lr_{}_seed_{}'.format(args.num_pos, args.batch_size, cfg.BASE_LR, args.seed)
...
imc_loss = IPDM(
    temp=0.07, lambda_var=0.1, lambda_sep=0.02, lambda_local=0.1,
    m_sep=0.1, normalize=True, topk_sep=20
).to(device)

criterion_id.to(device)
criterion_GE_CGA.to(device)
...
def imc_weight(epoch, T=32, warm=3, hold=24, w_max=0.3):
    import math
    if epoch < warm:
        return w_max * 0.5*(1 - math.cos(math.pi*epoch/max(1,warm)))
    if epoch < hold:
        return w_max
    t = (epoch - hold) / max(1, T - hold)
    t = max(0.0, min(1.0, t))
    return w_max * 0.5*(1 + math.cos(math.pi*t))
...
def train(epoch):
    ...
    for batch_idx, (input10, input11, input20, input21, label1, label2) in enumerate(trainloader):
        with amp.autocast(enabled=True):
            ...
            loss_GE_CGA = criterion_GE_CGA(feats, labels)
            
            loss_ids = criterion_id(clstemp,labels)
            loss_tris   = criterion_tri(feattemp, labels)
            loss_GE_CGAs = criterion_GE_CGA(feattemp, labels)

            with torch.cuda.amp.autocast(False):
                L_imc, imc_stat = imc_loss(
                    feats.float(), labels, parts=[feat_x1.float(), feat_x2.float(), feat_x3.float()]
                )
            w_imc = imc_weight(epoch, T=32, warm=3, hold=24, w_max=0.3)

            loss = (loss_tri + loss_triL1 + loss_triL2 + loss_triL3 \
                    + loss_id + loss_idL1 + loss_idL2 + loss_idL3) / 4 \
                   + (loss_ids + loss_tris + loss_GE_CGAs*0.3) * 0.5 \
                   + loss_GE_CGA*0.3 + L_imc * w_imc*0.5
        ...
        CAL_loss.update(loss_GE_CGA.item(), 2 * input10.size(0))
